...
The generator uses host tools and fails clearly when they are missing.
...
The generator searches `iqtree3`, `iqtree2`, then `iqtree`, but real generation also runs a small AliSim smoke test and uses the first binary whose AliSim path succeeds. This allows a working IQ-TREE 2.4.0 install to be used if a newer host binary is present but cannot run AliSim on the current machine.
...
9. The wrapper writes a summary JSON containing provenance, commands, tool versions, thresholds, counts, and output paths.
