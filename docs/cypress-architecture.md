...
- QA tracker rows can sync reviewed user stories to GitHub through `.github/workflows/user-story-issues.yml`. The workflow is manual-only, defaults to dry-run mode, and publishes rows with `story_sync=true`; for the initial GitHub review batch, use the `publish_all` workflow input to publish every enriched QA tracker row without changing the CSV flags. The story sync uses the enriched `story_*`, `tested_*`, and `source_confidence` columns in `docs/*qa-tracker.csv`, keys issues by QA tracker source plus row ID, and preserves reviewer notes outside the generated issue block on later syncs. Set repository secret `USER_STORY_PROJECT_TOKEN` for GitHub Projects write access and repository variables `USER_STORY_PROJECT_OWNER`, `USER_STORY_PROJECT_NUMBER`, and optionally `USER_STORY_PROJECT_OWNER_TYPE`, `USER_STORY_PROJECT_STATUS`, `USER_STORY_PROJECT_STATUS_FIELD`, and `USER_STORY_ASSIGNEES`. The project status defaults to `User Stories`.
