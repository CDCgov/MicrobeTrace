...
export class MicrobeTraceNextHomeComponent extends AppComponentBase implements AfterViewInit, OnInit, OnDestroy {
  ...
  public on_eula_reject(): void {
    ...
    private schedulePendingDashboardRestore(attempt: number = 0): void {
        ...
        const twoDView = this.commonService.visuals.twoD;
        const twoDReady = Boolean(
            twoDView?.isRendererReady?.()
            || (
                (twoDView?.cy || (window as any).cytoscapeInstance)
                && !(typeof (twoDView?.cy || (window as any).cytoscapeInstance)?.destroyed === 'function'
                    && (twoDView?.cy || (window as any).cytoscapeInstance).destroyed())
            )
        );
