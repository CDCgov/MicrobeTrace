...
describe('Journey Flow - Gantt uploaded node plus link data', () => {
  ...
  it('renders a symptom-window Gantt entry for uploaded nodes with complete symptom dates', () => {
    ...
    cy.get('ganttcomponent #gantt .y-axis-text').should('have.length', expectedRenderedBars);
    ...
    cy.window().then((win: any) => {
      ...
      expect(Object.keys(gantt.ganttChartData[0].timelines), 'timelines keyed by node id').to.have.length(expectedRenderedBars);
      expect(gantt.ganttChartService.ganttPhases, 'rendered row labels').to.have.length(expectedRenderedBars);
    });
