...
describe('File Handling and Processing', () => {
  ...
  it('preserves colliding node and link table columns without overriding canonical fields', () => {
    const collisionNodeFile = 'ImportCollisionNodes.csv';
    const collisionLinkFile = 'ImportCollisionLinks.csv';

    cy.loadFiles([
      { name: collisionNodeFile, datatype: 'node', field1: 'case_id', field2: 'None' },
    ...
    cy.window().then((win: any) => {
      ...
      const node = session.data.nodes.find((candidate: any) => candidate._id === 'CaseA');
      const link = session.data.links.find((candidate: any) =>
        candidate.source === 'CaseA' && candidate.target === 'CaseB',
      );
      ...
      expect(session.data.nodeFields).not.to.include.members(['Origin', 'Visible', 'Cluster']);
      expect(session.data.nodeFields.some((field: string) => field.startsWith('node_table_'))).to.equal(false);
      expect(session.data.linkFields).to.include.members(['Link Source', 'Link Target', 'Link Distance', 'Label', 'Link Origin', 'Link Directed']);
      expect(session.data.linkFields).not.to.include.members(['Source', 'Target', 'Distance', 'Origin', 'Directed']);
      expect(session.data.linkFields.some((field: string) => field.startsWith('link_table_'))).to.equal(false);

      expect(node._id).to.equal('CaseA');
      expect(node.id).to.equal('CaseA');
      expect(node['Node _ID']).to.equal('Source underscore A');
      expect(node['Node ID']).to.equal('Source id A');
      expect(node.Label).to.equal('Node label A');
      expect(node['Node Origin']).to.equal('NodeSource.csv');
      expect(node['Node Visible']).to.equal('false');
      expect(node['Node Cluster']).to.equal('99');
      expect(node.X).to.equal('12');
      expect(node.Y).to.equal('34');
      expect(node.Shape).to.equal('star');
      expect(node.origin[0]).to.equal(collisionNodeFile);
      expect(node.origin).to.include(collisionNodeFile);
      expect(node.origin).to.include(collisionLinkFile);
      expect(node.origin).not.to.include('NodeSource.csv');
      expect(node.visible).to.equal(true);
      expect(node.cluster).not.to.equal('99');

      expect(link.source).to.equal('CaseA');
      expect(link.target).to.equal('CaseB');
      expect(link.distance).to.equal(5);
      expect(link.hasDistance).to.equal(true);
      expect(link['Link Source']).to.equal('CaseA');
      expect(link['Link Target']).to.equal('CaseB');
      expect(link['Link Distance']).to.equal('5');
      expect(link.Label).to.equal('Link label AB');
      expect(link['Link Origin']).to.equal('LinkSource.csv');
      expect(link['Link Directed']).to.equal('true');
      expect(link.origin).to.deep.equal([collisionLinkFile]);
      expect(link.origin).not.to.include('LinkSource.csv');
      expect(link.directed).to.equal(false);
      expect(link.cluster).not.to.equal('77');
      expect(link.originColumnFromFile).to.deep.equal(['LinkSource.csv']);
    });
  });
